...
func Test_numberOfArithmeticSlicesV2(t *testing.T) {
	type args struct {
		nums []int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := numberOfArithmeticSlicesV2(tt.args.nums); got != tt.want {
				t.Errorf("numberOfArithmeticSlicesV2() = %v, want %v", got, tt.want)
			}
		})
	}
}
