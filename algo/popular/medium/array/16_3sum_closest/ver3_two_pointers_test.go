...
func Test_threeSumClosestV3(t *testing.T) {
	type args struct {
		nums   []int
		target int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := threeSumClosestV3(tt.args.nums, tt.args.target); got != tt.want {
				t.Errorf("threeSumClosestV3() = %v, want %v", got, tt.want)
			}
		})
	}
}
