...
func Test_kthPalindromeV2(t *testing.T) {
	type args struct {
		queries   []int
		intLength int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := kthPalindromeV2(tt.args.queries, tt.args.intLength); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("kthPalindromeV2() = %v, want %v", got, tt.want)
			}
		})
	}
}
