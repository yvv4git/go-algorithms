...
func Test_calcEquationV3(t *testing.T) {
	type args struct {
		equations [][]string
		values    []float64
		queries   [][]string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := calcEquationV3(tt.args.equations, tt.args.values, tt.args.queries); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("calcEquationV3() = %v, want %v", got, tt.want)
			}
		})
	}
}
