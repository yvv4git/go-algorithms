...
func Test_calcEquationV2(t *testing.T) {
	type args struct {
		equations [][]string
		values    []float64
		queries   [][]string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := calcEquationV2(tt.args.equations, tt.args.values, tt.args.queries); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("calcEquationV2() = %v, want %v", got, tt.want)
			}
		})
	}
}
