...
func Test_countNumbersWithUniqueDigitsV2(t *testing.T) {
	type args struct {
		n int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := countNumbersWithUniqueDigitsV2(tt.args.n); got != tt.want {
				t.Errorf("countNumbersWithUniqueDigitsV2() = %v, want %v", got, tt.want)
			}
		})
	}
}
