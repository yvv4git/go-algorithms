...
func Test_rotateRightV2(t *testing.T) {
	type args struct {
		head *ListNode
		k    int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := rotateRightV2(tt.args.head, tt.args.k); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("rotateRightV2() = %v, want %v", got, tt.want)
			}
		})
	}
}
