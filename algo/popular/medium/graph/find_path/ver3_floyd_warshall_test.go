...
func Test_validPathV3(t *testing.T) {
	type args struct {
		n           int
		edges       [][]int
		source      int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := validPathV3(tt.args.n, tt.args.edges, tt.args.source, tt.args.destination); got != tt.want {
				t.Errorf("validPathV3() = %v, want %v", got, tt.want)
			}
		})
	}
}
