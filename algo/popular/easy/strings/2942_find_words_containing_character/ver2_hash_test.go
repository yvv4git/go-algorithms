...
func Test_findWordsContainingV2(t *testing.T) {
	type args struct {
		words []string
		x     byte
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := findWordsContainingV2(tt.args.words, tt.args.x); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("findWordsContainingV2() = %v, want %v", got, tt.want)
			}
		})
	}
}
