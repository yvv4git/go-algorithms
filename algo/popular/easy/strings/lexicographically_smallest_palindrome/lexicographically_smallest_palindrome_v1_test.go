...
func Test_makeSmallestPalindromeV1(t *testing.T) {
	type args struct {
		s string
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := makeSmallestPalindromeV1(tt.args.s); got != tt.want {
				t.Errorf("makeSmallestPalindromeV1() = %v, want %v", got, tt.want)
			}
		})
	}
}
