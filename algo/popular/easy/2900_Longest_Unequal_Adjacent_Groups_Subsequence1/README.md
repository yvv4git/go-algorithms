...
## Объяснение
**Суть задачи:** 
Даны массив строк `words` и бинарный массив `groups`. Нужно найти самую длинную подпоследовательность из `words`, 
где соседние элементы имеют разные значения в `groups` (чередование 0 и 1).
**Методы решения:**
1. **Жадный алгоритм** — оптимальный подход. Проходим по массиву и добавляем элемент, если его группа отличается от предыдущего выбранного. Сложность: O(n).
2. **Динамическое программирование** — избыточно для данной задачи, так как жадный подход всегда оптимален.
3. **Два прохода** — альтернативный вариант: начинаем с группы 0 или 1 и выбираем лучший результат.
