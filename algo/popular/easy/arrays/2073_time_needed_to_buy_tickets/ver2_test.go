...
func Test_timeRequiredToBuyV2(t *testing.T) {
	type args struct {
		tickets []int
		k       int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotRes := timeRequiredToBuyV2(tt.args.tickets, tt.args.k); gotRes != tt.want {
				t.Errorf("timeRequiredToBuyV2() = %v, want %v", gotRes, tt.want)
			}
		})
	}
}
